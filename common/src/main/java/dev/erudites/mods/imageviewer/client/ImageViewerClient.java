...
public final class ImageViewerClient {
    ...
    public static void tick(final Minecraft minecraft) {
        ...
        while (OPEN_KEY.consumeClick()) {
            if (catalog != null && minecraft.screen == null) {
    ...
    private static void openViewer(final Minecraft minecraft, final CatalogPayload catalog) {
        ...
        if (categories.size() == 1) {
            minecraft.setScreen(new ImageViewerScreen(categories.getFirst().images(), catalog.keepAspectRatio()));
        } else {
            minecraft.setScreen(new ImageViewerSelectionScreen(categories, catalog.keepAspectRatio()));
        }
    }
